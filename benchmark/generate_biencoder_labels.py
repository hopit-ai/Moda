...
FINETUNED_MODEL_DIR = _REPO_ROOT / "models" / "moda-fashionclip-finetuned" / "best"


def retrieve_candidates(
    queries: list[tuple[str, str]],
    top_k: int = TOP_K_RETRIEVE,
    retriever_model: str = "baseline",
) -> dict[str, list[tuple[str, int]]]:
    """Run retrieval to get top-K candidates per query.

    retriever_model="baseline" uses the base FashionCLIP FAISS index.
    retriever_model="finetuned" uses the fine-tuned bi-encoder for Round 2 mining.
    ...
    if retriever_model == "finetuned" and FINETUNED_MODEL_DIR.exists():
        log.info("Loading FINE-TUNED bi-encoder for Round 2 mining...")
        import open_clip
        model, _, _ = open_clip.create_model_and_transforms("hf-hub:Marqo/marqo-fashionCLIP")
        tokenizer = open_clip.get_tokenizer("hf-hub:Marqo/marqo-fashionCLIP")
        state = torch.load(FINETUNED_MODEL_DIR / "model_state_dict.pt", map_location=device)
        ...
        if retriever_model == "finetuned":
            log.warning("Fine-tuned model not found at %s, falling back to baseline",
                        FINETUNED_MODEL_DIR)
...
def main():
    ...
    parser.add_argument("--retriever_model", type=str, default="baseline",
                        choices=["baseline", "finetuned"],
                        help="Use 'finetuned' for Round 2 iterative HN mining")
    parser.add_argument("--output", type=str, default=None,
                        help="Custom output path (default: biencoder_retriever_labels.jsonl)")
    ...
    log.info("Strategy: %s retrieval → %s labeling",
             args.retriever_model, args.model)
    log.info("=" * 60)
    ...
    if args.retriever_model == "finetuned" and args.output is None:
        output_path = PROCESSED_DIR / "biencoder_retriever_labels_r2.jsonl"
        log.info("Round 2 output → %s", output_path)

    retrieval_results = retrieve_candidates(
        queries, top_k=args.top_k, retriever_model=args.retriever_model)
