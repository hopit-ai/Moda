...
AUDIT_MODEL = "openai/gpt-5-mini"

RANDOM_SEED = 42
HARD_NEGS_PER_QUERY = 5
RANDOM_NEGS_PER_QUERY = 1
MAX_CONCURRENT = 20
MAX_RETRIES = 3
...
def build_article_texts(articles_df: pd.DataFrame) -> dict[str, str]:
    """Same format as train_cross_encoder.py for consistency."""
    texts = {}
...
async def label_one(
    ...
    for attempt in range(MAX_RETRIES):
        try:
            tok_limit = 500 if "gpt-5" in model else 200
            kwargs: dict = dict(
                model=model,
