...
def load_triplets(
    ...
    if not val_triplets:
        raise ValueError(
            "No val-split triplets found. Ensure query_splits.json has val query IDs "
            "that overlap with the label file. Refusing to carve validation from "
            "training data (causes leakage)."
        )
