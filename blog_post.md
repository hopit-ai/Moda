...
**NER attribute boosting** uses zero-shot named entity recognition to extract fashion attributes from queries. "Navy slim fit jeans mens" becomes {color: navy, fit: slim, type: jeans, gender: mens}. Those attributes get mapped to H&M's product fields and used as relevance boosts. We tested both [GLiNER](https://github.com/urchade/GLiNER) (`urchade/gliner_medium-v2.1`, [NAACL 2024](https://aclanthology.org/2024.naacl-long.300/)) and [GLiNER2](https://github.com/fastino-ai/GLiNER2) (`fastino/gliner2-base-v1`, EMNLP 2025). GLiNER2 produces better entity extractions and improves BM25+NER by +16% over v1. In the full pipeline with dense retrieval and cross-encoder reranking on top, the gap narrows to +0.8% (nDCG@10: v1=0.0549, v2=0.0553). That's a zero-shot result though. Better entity extraction becomes more valuable in the fine-tuning stages: cleaner attribute labels mean better training data for field-specific encoders, more precise hard negative mining, and higher quality LLM labeling prompts. The NER improvement pays off downstream even when the off-the-shelf reranker masks it at retrieval time.
